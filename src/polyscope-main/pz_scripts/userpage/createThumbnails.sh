...
# Version: 0.0.5 - FIXED path handling
SETUPFILE=$1
SETUPPATH=$(dirname "$SETUPFILE")  # FIX: Use dirname instead of parameter expansion
...
if [[ $DZI == *"/var/www/polyzoomer/"* ]]; then
    # Direct polyzoomer path
    PATHTODZI="${DZI/.dzi/}"
    PATHTODZIFILES="${PATHTODZI}_files/"
elif [[ $DZI == *"/customers/"* ]]; then
...
if [[ -d "$PATHTODZIFILES" ]]; then
    ...
    LOOPCOUNTER=$DIRECTORYCOUNT  # Force fallback if directory doesn't exist
fi
