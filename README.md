# Polyscope without SSL certificate (under development)
...
### Clean
```sh
pkill -9 vips; pkill -f "polyzoom"; pkill -f "executeSupervisor"; pkill -f "doPolyzoom"; killall -9 php 2>/dev/null; rm -rf /tmp/vips-* 2>/dev/null; find /var/www -name "*.lock" -delete 2>/dev/null; find /var/www -name "*.tmp" -delete 2>/dev/null; echo "All processing jobs killed and temp files cleaned"
```

### Known Issues

Due to file system access, it is not recommended to manually copy files to the internal storage directory. This will cause job to stall as the backend might not have the access to the file. 
